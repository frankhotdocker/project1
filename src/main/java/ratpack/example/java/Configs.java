...
public class Configs {
    ...
    public static void main(String[] args) {
        ...
        //Config test = newBuilder().withSystemEnvironment().withSystemProperties().withOptionalFile("/home/frank/test/project1/src/main/resources/application.frank.conf").build();

        //Config application_conf = ConfigFactory.load();
        //log.info("name: {}", application_conf.getString("pools.default.minimumIdle"));
        ...
        Config envBased = ConfigFactory.systemEnvironment()
                .withFallback(fallbackConfig)
                .resolve();

        log.info("name: {}", defaultConfig.getString("conf.name"));
        log.info("name: {}", envBased.getString("conf.name"));
